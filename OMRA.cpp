...
double stringSimilarity(string origin, string comparison) {
	...
	const unsigned long ORIGIN_LENGTH = origin.length() + 1;
	const unsigned long COMPARISON_LENGTH = comparison.length() + 1;
	vector< vector<int> > matrix;
	matrix.resize(ORIGIN_LENGTH, vector<int>(COMPARISON_LENGTH, 0));
